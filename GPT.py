    ...
    def load_prompt(self, parser):
        if self.args.dataset == 'odido':
            prompt_cate = """
                ...
                1. 
                sentence: de klant belt om een probleem met de internetverbinding te melden
                output: 
                {{ 'quads': [
                    {{'aspect': 'internetverbinding', 'category': 'netwerk', 'opinion': 'probleem', 'sentiment': 'negatief'}}
                    ]
                }}
                
                2. 
                sentence: de klant belde omdat de simkaart van haar mobiele telefoon geblokkeerd was en ze haar nummer niet meer wist
                output: 
                {{ 'quads': [
                    {{'aspect': 'simkaart', 'category': 'beveiliging', 'opinion': 'geblokkeerd', 'sentiment': 'negatief'}}
                    ] 
                }}
                   
                3. 
                sentence: ondanks meerdere bezoeken van monteurs en het gebruik van wifiversterkers werkt de wifi aan de linkerkant van het huis nog steeds niet
                output: 
                {{ 'quads': [
                    {{'aspect': 'monteurs', 'category': 'installatie', 'opinion': 'ondanks', 'sentiment': 'negatief'}}, 
                    {{'aspect': 'wifi', 'category': 'netwerk', 'opinion': 'niet', 'sentiment': 'negatief'}}
                    ] 
                }}
                
                4. 
                sentence: de klant belt omdat hij nog steeds geen tv en internet thuis heeft ondanks eerdere beloftes van een monteur
                output: 
                {{ 'quads': [
                    {{'aspect': 'tv', 'category': 'netwerk', 'opinion': 'geen', 'sentiment': 'negatief'}}, 
                    {{'aspect': 'internet', 'category': 'netwerk', 'opinion': 'geen', 'sentiment': 'negatief'}}, 
                    {{'aspect': 'monteur', 'category': 'installatie', 'opinion': 'ondanks', 'sentiment': 'negatief'}}
                    ] 
                }}
                
                5. 
                sentence: de agent heeft bevestigd dat de punten zijn teruggestuurd en heeft een krediet ingediend om de boete te annuleren
                output: 
                {{ 'quads': [
                ...
                6. 
                sentence: de klant belde omdat er iets fout was gegaan bij het bestellen van een abonnement
                output: 
                {{ 'quads': [
                    {{'aspect': 'bestellen', 'category': 'levering', 'opinion': 'fout', 'sentiment': 'negatief'}}
                    ] 
                }}
                
                7. 
                sentence: de klant is tevreden met het internet gebruikt de vaste telefoon niet veel maar vindt het handig om te hebben en heeft goedkope mobiele abonnementen met korting 
                output: 
                {{ 'quads' [
                    {{'aspect': 'internet', 'category': 'netwerk', 'opinion': 'tevreden', 'sentiment': 'positief'}}
                    ] 
                }}

                8. 
                sentence: de agent maakt een notitie en vraagt de klant om over twee uur opnieuw een speedtest uit te voeren en indien het probleem aanhoudt terug te bellen
                output: 
                {{ 'quads': [
                    {{'aspect': '', 'category': '', 'opinion': '', 'sentiment': ''}}
                    ]
                ...
                9. 
                sentence: de klant belt om een bedrag over te maken naar de inkasso zakelijk van de telecombedrijf
                output: 
                {{ 'quads': [
                    {{'aspect': '', 'category': '', 'opinion': '', 'sentiment': ''}}
                    ]
                ...
                10. 
                sentence: de agent constateert een storing in de bekabelde desktop van de klant en adviseert om deze opnieuw op te starten
                output:
                {{ 'quads': [
                    {{'aspect': 'desktop', 'category': 'netwerk', 'opinion': 'storing', 'sentiment': 'negatief'}}
                    ] 
                }}
